...
async function fetchGalleryUrl(itemId: string): Promise<string | null> {
  ...
  try {
    ...
    if (!body.includes("<Ack>Success</Ack>") && !body.includes("<Ack>Warning</Ack>")) {
      ...
      console.error(`[sales/GetItem] non-success for ${itemId}:`, body.slice(0, 1000));
      return null;
    }
    ...
    if (!gallery) {
      // TEMP DIAGNOSTIC (remove once photo lookup is confirmed working):
      console.error(
        `[sales/GetItem] no GalleryURL for ${itemId}. PictureDetails block:`,
    ...
    // TEMP DIAGNOSTIC (remove once photo lookup is confirmed working):
    console.error(`[sales/GetItem] threw for ${itemId}:`, (err as Error).message);
    return null;
  }
}
